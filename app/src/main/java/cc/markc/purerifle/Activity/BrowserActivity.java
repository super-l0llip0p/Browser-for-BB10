...
import cc.markc.purerifle.Browser.AdBlock;
...
import cc.markc.purerifle.Service.ClearDataService;
import cc.markc.purerifle.Task.ScreenshotTask;
import cc.markc.purerifle.Database.Record;
import cc.markc.purerifle.Database.RecordAction;
import cc.markc.purerifle.Service.HolderService;
...
import cc.markc.purerifle.Unit.ViewUnit;
import cc.markc.purerifle.View.*;
...
public class BrowserActivity extends Activity implements BrowserController {
    ...
    private ImageButton relayoutOK;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        Log.d("BrowserActivity", "1");
        if (requestCode == IntentUnit.REQUEST_URL) {
            if (resultCode != AppCompatActivity.RESULT_OK) {
                Log.d("BrowserActivity", "2");
            } else {
                Log.d("BrowserActivity", "3");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!QbSdk.canLoadX5(getApplicationContext())) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    boolean result = QbSdk.preinstallStaticTbs(getApplicationContext());
                    Log.i("BrowserActivity", "InstallTencentX5: " + result);
                }
            }).start();
        }
        ...
        initOmnibox();
        initSearchPanel();
        relayoutOK = (ImageButton) findViewById(R.id.main_relayout_ok);
    ...
    private void dispatchIntent(Intent intent) {
        ...
        if (intent != null && intent.hasExtra(IntentUnit.OPEN)) { // From HolderActivity's menu
            ...
            if (sp.getBoolean(getString(R.string.sp_first), true)) {
                pinAlbums(BrowserUnit.BASE_URL + BrowserUnit.INTRODUCTION);
    ...
    public void onDestroy() {
        ...
        if (sp.getBoolean(getString(R.string.sp_clear_quit), false)) {
            Intent toClearService = new Intent(this, ClearDataService.class);
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP || keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
            SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(this);
            int vc = Integer.valueOf(sp.getString(getString(R.string.sp_volume), "2"));
            if (vc != 2) {
                return true;
            }
        }
    ...
    private void initSwitcherView() {
        ...
        switcherSetting.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                startActivityForResult(intent, IntentUnit.REQUEST_URL);
            }
        });
    ...
    private boolean showOverflow() {
        ...
        ContextThemeWrapper contextThemeWrapper = new ContextThemeWrapper(BrowserActivity.this, R.style.PopupMenuTheme);
        PopupMenu popupMenu = new PopupMenu(contextThemeWrapper, omniboxOverflow);
        ...
        if (currentAlbumController != null && currentAlbumController instanceof NinjaRelativeLayout) {
            ...
            if (ninjaRelativeLayout.getFlag() == BrowserUnit.FLAG_HOME) {
                ...
                popupMenu.show();
                CommonUnit.showPopupMenuIcon(popupMenu);
            }
        } else if (currentAlbumController != null && currentAlbumController instanceof NinjaWebView) {
            popupMenu.getMenuInflater().inflate(R.menu.menu_webpage, popupMenu.getMenu());
            popupMenu.show();
            CommonUnit.showPopupMenuIcon(popupMenu);
        }
        ...
        popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem menuItem) {
                ...
                if (id == R.id.back_to_top) {
                    NinjaWebView ninjaWebView = (NinjaWebView) currentAlbumController;
                    ninjaWebView.getView().scrollTo(ninjaWebView.getScrollX(), 0);
                } else if (id == R.id.add_to_speed_dial) {
                    NinjaWebView ninjaWebView = (NinjaWebView) currentAlbumController;
                    RecordAction action = new RecordAction(BrowserActivity.this);
                    action.open(true);
